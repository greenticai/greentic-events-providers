//! SendGrid email event provider component.
//!
//! Provides SendGrid email event processing functionality.

#![deny(unsafe_op_in_unsafe_fn)]

mod bindings {
    wit_bindgen::generate!({
        path: "wit/events-provider-email-sendgrid",
        world: "component-v0-v6-sendgrid",
        generate_all
    });
}
...
pub(crate) const PROVIDER_ID: &str = "events-provider-email-sendgrid";
...
impl Default for ProviderConfig {
    fn default() -> Self {
        Self {
            ...
            from: None,
            persistence_key_prefix: Some("events/email/sendgrid/queued".into()),
        }
    }
}
...
mod tests {
    ...
    fn apply_answers_produces_config() {
        let answers = json!({
            "enabled": "true",
            "messaging_provider_id": "my-sendgrid-provider",
        ...
        assert!(cfg.enabled);
        assert_eq!(cfg.messaging_provider_id, "my-sendgrid-provider");
