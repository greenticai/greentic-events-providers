//! SMS event provider component.
//!
//! Provides SMS event processing functionality.

#![deny(unsafe_op_in_unsafe_fn)]

mod bindings {
    wit_bindgen::generate!({
        path: "wit/events-provider-sms",
...
pub(crate) const PROVIDER_ID: &str = "events-provider-sms";
...
impl Default for ProviderConfig {
    fn default() -> Self {
        Self {
            ...
            from: None,
            persistence_key_prefix: Some("events/sms/queued".into()),
        }
    }
}
...
mod tests {
    ...
    fn apply_answers_produces_config() {
        let answers = json!({
            "enabled": "true",
            "messaging_provider_id": "my-sms-provider",
            "from": "+15551234",
        });
        let out = apply_answers_impl(&answers);
        assert!(out.ok);
        ...
        assert!(cfg.enabled);
        assert_eq!(cfg.messaging_provider_id, "my-sms-provider");
